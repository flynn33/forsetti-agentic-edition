...
Phase 00B inventoried the local MCP/tooling surface and created an allowlist. The remediation package catalog lists 23 desired MCP capabilities. This host has 10 enabled local MCP entries in the user-level local host config, all using `npx` over stdio. This already-running session does not expose those entries as active callable namespaces, so Phase 00B used local equivalents for the required checks.
...
| `sqlite` | `npx -y mcp-sqlite C:\Users\james\.local-config\sqlite\forsetti-agentic-edition.sqlite` | `mcp-sqlite` 1.0.9, SHA-256 `5e3342051ddb34f42a3bc5dc278437530d8c8cc69b33b3d042b24ba65cdfba8d` | Local SQLite state file access. | Restricted to explicit local evidence database needs. |
...
- Any mutation to user-level local host config beyond the already recorded setup.
...
- `local-host mcp list` and `local-host mcp get <server>` succeeded for the configured entries.
